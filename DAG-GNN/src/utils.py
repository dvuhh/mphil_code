...
def load_data(args, batch_size=1000, suffix='', debug = False):
    ...
    if args.data_type == 'synthetic':
        ...
        file_path = os.path.join(args.data_dir, args.data_filename)
        X = np.loadtxt(file_path, delimiter=',')
        # Create an empty graph G, as it's a required return value
        G = nx.DiGraph()
    ...
    valid_data_loader = DataLoader(valid_data, batch_size=batch_size, drop_last=True)
