...
class SoftSort_p1(torch.nn.Module):
    # differentiable sorting operation
    ...
    def forward(self, scores: Tensor):
        ...
        scores: elements to be sorted. Typical shape: batch_size x n
        """
        # Add dimension for pairwise comparisons
        scores = scores.unsqueeze(-1) # [batch_size, n, 1]

        # Sort scores in descending order
        sorted = scores.sort(descending=True, dim=1)[0]

        # Compute pairwise differences and convert to probabilities
        ...
        if self.hard:
            ...
            # Straight-through estimator: use hard values in forward pass but soft in backward
            P_hat = (P - P_hat).detach() + P_hat
        return P_hat
