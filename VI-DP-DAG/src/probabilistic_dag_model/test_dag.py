import numpy as np
import torch
from src.metrics import edge_auroc, edge_apr, edge_fn_fp_rev, SHD, SID, edge_mcc
...
def test(model, true_dag_adj):
    ...
    metrics = {'undirected_edge_auroc': edge_auroc(pred_edges=prob_mask + prob_mask.T, true_edges=true_dag_adj + true_dag_adj.T),
               ...
               'edge_mcc': edge_mcc(pred_edges=prob_mask, true_edges=true_dag_adj),
               'undirected_edge_mcc': edge_mcc(pred_edges=prob_mask + prob_mask.T, true_edges=true_dag_adj + true_dag_adj.T),
               }
